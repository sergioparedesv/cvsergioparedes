...
function initCertifications() {
    ...
    const loadFrame = (details) => {
        const iframe = details.querySelector(':scope > .cert-course-body .cert-pdf-frame, :scope > .cert-featured-pdf .cert-pdf-frame');
        if (iframe && !iframe.getAttribute('src') && iframe.dataset.src) {
            iframe.setAttribute('src', iframe.dataset.src);
        }
    };

    certSection.querySelectorAll('details').forEach(details => {
        details.addEventListener('toggle', () => {
            if (details.open) loadFrame(details);
        });
    });

    const expandBtn = document.getElementById('certExpandAll');
    const collapseBtn = document.getElementById('certCollapseAll');
